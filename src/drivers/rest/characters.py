...
@router.get("/by-user/{user_tg_id}", response_model=CharacterResponse)
@inject
async def get_character_by_user(
    ...
    """Получить персонажа по telegram ID пользователя"""
    try:
        character = await use_case.execute(user_tg_id)
        return CharacterResponse.model_validate(character)
    except EntityNotFoundException as e:
        raise NotFoundException(detail=str(e))
