...
class SQLAlchemyItemCategoriesRepository(
    ...
    async def add(self, category: ItemCategory) -> ItemCategory:  # type: ignore[override]
        model = ItemCategoryModel(
            id=category.id,
            name=category.name,
    ...
    async def update(self, category: ItemCategory) -> ItemCategory:  # type: ignore[override]
        async with self._uow() as uow:
            model = await uow.session.get(ItemCategoryModel, category.id)
            if model is None:
    ...
    async def delete(self, category_id: uuid.UUID) -> None:  # type: ignore[override]
        async with self._uow() as uow:
...
class SQLAlchemyItemsRepository(SQLAlchemyRepository[ItemModel], ItemsRepository):
    ...
    async def get(self, item_id: uuid.UUID) -> Item | None:  # type: ignore[override]
    ...
    async def add(self, item: Item) -> Item:  # type: ignore[override]
        model = ItemModel(
            id=item.id,
    ...
    async def update(self, item: Item) -> Item:
        async with self._uow() as uow:
            ...
            model.picture_url = item.picture_url
    ...
    async def delete(self, item_id: uuid.UUID) -> None:  # type: ignore[override]
        async with self._uow() as uow:
...
class SQLAlchemyBackgroundsRepository(
    ...
    async def get(self, background_id: uuid.UUID) -> Background | None:  # type: ignore[override]
    ...
    async def add(self, background: Background) -> Background:  # type: ignore[override]
        model = BackgroundModel(
            id=background.id,
    ...
    async def delete(self, background_id: uuid.UUID) -> None:  # type: ignore[override]
        async with self._uow() as uow:
